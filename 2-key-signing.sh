...
bob_runs  gpg -e -s -r alice@example.com msg-to-alice
# Use --armor to make it ASCII-encoded and pastable
...
# The signature is there, but the key that signed it is not
alice_runs  gpg --decrypt msg-to-alice.asc  |& highlight "not certified"

# Alice asks Bob for his key's fingerprint, verifies it and signs the key
